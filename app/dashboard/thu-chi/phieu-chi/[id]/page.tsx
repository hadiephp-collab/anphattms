...
export default function PhieuChiDetailPage() {
  ...
  useEffect(() => {
    ...
    transactionsApi.getOne(Number(id))
      .then(setTx)
      .catch(() => setError('Không tìm thấy phiếu chi'))
      .finally(() => setLoading(false));
  }, [id]);

  async function handleDelete() {
    if (!confirm('Bạn có chắc muốn hủy phiếu chi này? Hành động này không thể hoàn tác.')) return;
    setDeleting(true);
    try {
      await transactionsApi.remove(Number(id));
      router.push('/dashboard/thu-chi/phieu-chi');
    } catch {
  ...
  if (error || !tx) return (
    <div className="flex flex-col items-center justify-center h-64 gap-3">
      ...
      <button onClick={() => router.back()} className="text-sm text-red-600 hover:underline">← Quay lại</button>
    </div>
  );
          ...
          {!tx.isDeleted && (
            <>
              ...
              <button onClick={handleDelete} disabled={deleting}
                className="flex items-center gap-2 px-4 py-2 text-sm font-medium border border-red-200 text-red-600 rounded-xl hover:bg-red-50 disabled:opacity-60 transition">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                {deleting ? 'Đang hủy...' : 'Hủy phiếu'}
              </button>
            </>
          )}
          <button onClick={() => window.open(`/print/phieu-chi/${tx.id}`, '_blank')}
            className="flex items-center gap-2 px-4 py-2 text-sm font-medium border border-gray-200 rounded-xl hover:bg-gray-50 text-gray-600 transition">
      ...
      <div className="flex-1 overflow-auto px-6 py-5">
        <div className="flex gap-5 max-w-5xl mx-auto">
          ...
          <div className="flex-1 space-y-5">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
              ...
              <div className="grid grid-cols-2 gap-x-8 gap-y-4">
                <InfoRow label="Nhóm người nhận" value={tx.payerType ? PAYER_TYPE_LABEL[tx.payerType] || tx.payerType : '—'} />
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
              ...
              <div className="grid grid-cols-2 gap-x-8 gap-y-4">
                <InfoRow label="Giá trị"
                  value={<span className="text-red-500 font-bold text-base">{fmtMoney(Number(tx.amount))}</span>} />
                <InfoRow label="Hình thức thanh toán" value={PM_LABEL[tx.paymentMethod] || tx.paymentMethod} />
                <InfoRow label="Ngày chi" value={fmtDate(tx.date)} />
              ...
              <div className="mt-4 pt-4 border-t border-gray-50 space-y-2.5">
                <CheckRow label="Hạch toán kết quả kinh doanh" checked={tx.affectsBusinessResult !== false} accent="red" />
              </div>
            </div>
              ...
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                ...
                <div className="flex flex-wrap gap-2">
                  {tx.tags.split(',').map(t => t.trim()).filter(Boolean).map(t => (
                    <span key={t} className="px-2.5 py-1 text-sm bg-red-50 text-red-700 border border-red-100 rounded-lg">{t}</span>
                  ))}
                </div>
              </div>
            )}
          ...
          <div className="w-72 shrink-0 space-y-4">
            ...
            {tx.note && (
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                <h2 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Mô tả / Diễn giải</h2>
            ...
            {tx.imageUrl && (
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                <h2 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">Ảnh chứng từ</h2>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl mx-4 overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <div>
            <h2 className="text-base font-bold text-gray-800">Sửa phiếu chi</h2>
          ...
          <div className="space-y-4">
            ...
            <Field label="Diễn giải / Ghi chú">
              <textarea value={form.note} onChange={e => set('note', e.target.value)}
                rows={4} placeholder="Mô tả chi tiết nội dung chi tiêu..."
        ...
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-gray-100 bg-gray-50">
          ...
          <button onClick={handleSave} disabled={saving}
            className="px-5 py-2 text-sm font-semibold bg-red-600 text-white rounded-xl hover:bg-red-700 disabled:opacity-60 transition">
...
function CheckRow({ label, checked, accent }: { label: string; checked: boolean; accent: string }) {
  return (
    <label className="flex items-center gap-2.5">
      <div className={`w-4 h-4 rounded flex items-center justify-center ${checked ? `bg-${accent}-500` : 'bg-gray-200'}`}>
