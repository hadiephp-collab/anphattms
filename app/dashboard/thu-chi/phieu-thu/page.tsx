...
interface PageRes { data: Tx[]; total: number; totalPages: number; summary: { totalReceipt: number; count: number }; }
...
const ALL_COLS = [
  ...
  { key: 'Nhóm đối tượng', label: 'Nhóm người nộp',          shortLabel: 'Nhóm NNộp',   defaultOn: false },
...
function renderThuCell(key: ColKey, tx: Tx): React.ReactNode {
  switch (key) {
    case 'Danh mục':
      return (
    ...
    case 'Đối tác':
      return (
        <td key={key} className="px-4 py-3">
          {tx.partner ? <a href={`/dashboard/partners/${tx.partner.id}`} className="text-emerald-700 hover:underline font-medium">{tx.partner.name}</a> : '—'}
        </td>
      );
    case 'PTTT':
          ...
          {tx.tags ? (
            <div className="flex flex-wrap gap-1">
              {tx.tags.split(',').map(t => t.trim()).filter(Boolean).map(t => (
                <span key={t} className="px-1.5 py-0.5 text-[11px] bg-emerald-50 text-emerald-700 border border-emerald-100 rounded-md">{t}</span>
              ))}
            </div>
          ) : '—'}
        </td>
    ...
    case 'Chứng từ':
      return (
        <td key={key} className="px-4 py-3 text-center">
          {tx.imageUrl
            ? <span title="Có ảnh chứng từ" className="inline-flex items-center justify-center w-6 h-6 bg-emerald-100 text-emerald-600 rounded-full">
    ...
    case 'Mã đối tác':
      return (
        <td key={key} className="px-4 py-3 font-mono text-xs">
          {tx.partner ? <a href={`/dashboard/partners/${tx.partner.id}`} className="text-gray-600 hover:text-emerald-700 hover:underline">{tx.partner.code || tx.partner.id}</a> : '—'}
        </td>
      );
    case 'Tham chiếu':
      ...
      return (
        <td key={key} className="px-4 py-3 text-center">
          {tx.isDeleted
            ...
            : <span className="px-2 py-1 text-xs bg-emerald-100 text-emerald-700 rounded-lg">Hoàn thành</span>}
        </td>
      );
    case 'Số tiền':
      return <td key={key} className="px-4 py-3 text-right font-semibold text-emerald-600 whitespace-nowrap">{fmtMoney(Number(tx.amount))}</td>;
    default: return null;
  }
}
...
export default function PhieuThuPage() {
  ...
  const [summary, setSummary] = useState({ totalReceipt: 0, count: 0 });
  ...
  const [visibleCols, setVisibleCols] = useState<Record<ColKey, boolean>>(() => {
    try { const s = localStorage.getItem('phieu_thu_cols'); if (s) return { ...defaultColVisibility, ...JSON.parse(s) }; } catch {}
    return defaultColVisibility;
  });

  const [colOrder, setColOrder] = useState<ColKey[]>(() => {
    try {
      const s = localStorage.getItem('phieu_thu_col_order');
      if (s) {
  ...
  function handleSaveColConfig(newVisible: Record<ColKey, boolean>, newOrder: ColKey[]) {
    ...
    try {
      localStorage.setItem('phieu_thu_cols', JSON.stringify(newVisible));
      localStorage.setItem('phieu_thu_col_order', JSON.stringify(newOrder));
    } catch {}
  }

  useEffect(() => { branchesApi.getAll().then(r => setBranches(r.data || r || [])).catch(() => {}); }, []);
  useEffect(() => { transactionGroupsApi.getAll('receipt').then((r: any) => setGroups(r || [])).catch(() => {}); }, []);

  const buildParams = useCallback((extra?: Record<string, string>) => {
    const params: Record<string, string> = { type: 'receipt', ...extra };
  ...
  function handleBulkPrint() {
    ...
    const rowsHtml = selected.map(tx => `
      <tr>
        ...
        <td style="text-align:right;color:#059669;font-weight:600">${fmtMoney(Number(tx.amount))}</td>
    ...
    const html = `<!DOCTYPE html><html lang="vi"><head><meta charset="UTF-8">
      ...
      <style>
        ...
        h2{color:#059669;margin:0 0 4px}p{color:#666;margin:0 0 16px;font-size:12px}
        table{width:100%;border-collapse:collapse}
        th{background:#f0fdf4;color:#374151;text-align:left;padding:8px 10px;font-size:11px;text-transform:uppercase;letter-spacing:.05em;border-bottom:2px solid #d1fae5}
        td{padding:8px 10px;border-bottom:1px solid #f3f4f6}
        .total{text-align:right;padding:12px 10px 0;font-weight:bold;color:#059669;font-size:15px}
        @media print{body{margin:0}}
      </style></head><body>
      <h2>Danh sách phiếu thu</h2>
      <p>In ngày: ${new Date().toLocaleDateString('vi-VN')} — Tổng: ${selected.length} phiếu</p>
      <table><thead><tr>
        <th>Ngày thu</th><th>Mã phiếu</th><th>Danh mục</th><th>Diễn giải</th>
        <th>Đối tác</th><th>PTTT</th><th>Số tiền</th><th>Trạng thái</th>
      </tr></thead><tbody>${rowsHtml}</tbody></table>
      <div class="total">Tổng thu: ${fmtMoney(totalAmt)}</div>
  ...
  async function handleExport() {
    ...
    try {
      ...
      a.href = url; a.download = `phieu-thu-${new Date().toISOString().slice(0, 10)}.csv`;
  ...
  const totalColCount = orderedVisible.length + 3; // checkbox + ngày + mã + visible cols
  ...
  return (
    <div className="flex flex-col h-full bg-[#f5f6fa]">
      ...
      <div className="flex items-center justify-between px-6 py-4 bg-white border-b border-gray-100">
        <div>
          <h1 className="text-xl font-bold text-gray-800">Phiếu Thu</h1>
          <div className="flex items-center gap-3 mt-1">
            <p className="text-xs text-gray-400">Quản lý các khoản thu tiền</p>
            <span className="text-gray-200">|</span>
            <Link href="/dashboard/thu-chi/loai-phieu-thu"
        ...
        <div className="flex items-center gap-2">
          ...
          <Link href="/dashboard/thu-chi/phieu-thu/tao-moi"
        ...
        <div className="flex gap-1">
          {[
            ...
            <button key={t.key} onClick={() => { setStatusTab(t.key as any); setPage(1); }}
              className={`px-4 py-2.5 text-sm font-medium rounded-t-lg border-b-2 transition ${statusTab === t.key ? 'border-emerald-500 text-emerald-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>
              {t.label}
            </button>
          ))}
        </div>
      ...
      <div className="flex items-center gap-2 px-6 py-3 bg-white border-b border-gray-100 flex-wrap">
        ...
        <select value={datePreset} onChange={e => applyPreset(e.target.value)}
          className="px-3 py-2 text-sm border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-400">
          ...
          {selectedIds.size > 0 && (
            <div className="flex items-center gap-3 px-5 py-2.5 bg-emerald-50 border-b border-emerald-100">
              <span className="text-sm font-semibold text-emerald-700">
                Đã chọn <span className="bg-emerald-600 text-white text-xs px-1.5 py-0.5 rounded-md">{selectedIds.size}</span> phiếu thu
              </span>
              {statusTab !== 'deleted' && (
            ...
            <table className="w-full min-w-max text-sm">
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr><td colSpan={totalColCount} className="py-16 text-center text-gray-400 text-sm">Chưa có phiếu thu nào</td></tr>
                ) : (
                  rows.map(tx => (
                    <tr key={tx.id} className={`border-b border-gray-50 hover:bg-gray-50 transition ${tx.isDeleted ? 'opacity-50' : ''} ${selectedIds.has(tx.id) ? 'bg-emerald-50/40' : ''}`}>
                      ...
                      <td className="px-4 py-3">
                        <a href={`/dashboard/thu-chi/phieu-thu/${tx.id}`}
                          className="font-mono text-emerald-700 font-semibold hover:underline hover:text-emerald-800">
                          {tx.code}
                        </a>
                      </td>
                      {orderedVisible.map(key => renderThuCell(key, tx))}
                    </tr>
                  ))
                )}
              </tbody>
      ...
      <div className="flex items-center justify-between px-6 py-3 bg-white border-t border-gray-100">
        <div className="flex items-center gap-4 text-xs text-gray-400">
            ...
            <div className="flex items-center border border-gray-200 rounded-lg overflow-hidden">
              {([20, 50, 100] as const).map(n => (
                <button key={n} onClick={() => { setLimit(n); setPage(1); }}
                  className={`px-2.5 py-1 text-xs font-medium transition border-r border-gray-200 last:border-r-0 ${limit === n ? 'bg-emerald-600 text-white' : 'text-gray-500 hover:bg-gray-50'}`}>
                  {n}
                </button>
              ))}
            </div>
          ...
          <span>
            ...
            {' '}trên tổng <span className="font-semibold text-gray-600">{total}</span> phiếu thu
          </span>
          <span className="text-gray-200">·</span>
          <span className="font-semibold text-emerald-600">Tổng thu: {fmtMoney(summary.totalReceipt)}</span>
        </div>
        {totalPages > 1 && (
              ...
              .map((item, idx) =>
                item === 'ellipsis'
                  ...
                  : <button key={item} onClick={() => setPage(item as number)}
                      className={`w-7 h-7 flex items-center justify-center rounded-lg text-xs font-medium transition ${page === item ? 'bg-emerald-600 text-white shadow-sm' : 'text-gray-500 hover:bg-gray-100'}`}>
                      {item}
                    </button>
              )}
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm mx-4 p-6">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500 mt-0.5">Sẽ hủy <span className="font-semibold text-red-600">{selectedIds.size}</span> phiếu thu đã chọn. Thao tác này không thể hoàn tác.</p>
              </div>
            </div>
