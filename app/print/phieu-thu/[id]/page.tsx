...
import { DEFAULT_TEMPLATE_PHIEU_THU } from '@/lib/default-templates';
...
export default function PrintPhieuThuPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      if (!tx) { setError('Không tìm thấy phiếu thu'); setReady(true); return; }
      const template = settings?.templatePhieuThu || DEFAULT_TEMPLATE_PHIEU_THU;
      const pSize = settings?.paperSizePhieuThu || 'A4';
      setPaperSize(pSize);
